...
page_5 <- function(){
  ...
  page(
    ...
    ui <- function(request){
      tagList(
        h1("This is Rice genome page",style="text-align:center"),
        nav_links,


# To add ui part for page5   

useShinyjs(),

column(8,offset = 5, titlePanel("Rice Alignment output")), # titlePanel

sidebarLayout(

sidebarPanel(

column(12,textInput("Gene","Gene Name (LOC_Os10g21248 or others)")),

# column(6,textInput("Gene_ID","Sobic")),

column(12,textInput("Chr","Chromosome (chr10 or others)")),
...
pickerInput(
  inputId = "id", 
  label = "Genomes (Select Nipponbare and others) :", 
  choices = c('Nipponbare', 'ZS97', 'rufipogon', 'R498', 'punctata', 'nivara',  'N22',  'MH63',  'meridionalis',  'Kitaake', 'IR8', 'glumipatula', 'glaberrima', 'DomSufid', 'brachyantha', 'Basmati334', 'barthii'),
  options = list(
    'actions-box' = TRUE, 
    size = 17,
...
fluidRow(
  ...
  #   column(4, plotOutput("plot_tree")),
   ...
   # verbatimTextOutput("info")
    ...
    }, # For ui function of page_5



# To add server function part for page5
...
dna <- readDNAStringSet("Oryza_sativa_Nipponbare_7.0.cds_1.fa"); ## Try CDS.1
...
##############
system("cat extract_syn_fa_Rice_1_06_01.pl > extract_syn_fa_Rice_1_06_01_Test4_copy_3.pl")
...
replace_pattern_in(file_contents="target_ch='chr10'",replace=Chr_update,file_pattern="Test4_copy_3",file_contents_perl = TRUE);

Gene_update0 <- paste('gene=',input$Gene,sep = "'");  ## from shiny input
Gene_update <- paste(Gene_update0,symbol,sep = "");
replace_pattern_in(file_contents="gene='LOC_Os10g21248'",replace=Gene_update,file_pattern="Test4_copy_3",file_contents_perl = TRUE);


##############
system("perl extract_syn_fa_Rice_1_06_01_Test4_copy_3.pl 1");


######################################################################## To search for the outliers!
result_folder1<-c('/mnt/946c1663-fcbd-4a78-8887-c55f23c5b496/bszhang/Rice/Nipponbare/Candidate_genes/');
...
output$text3 <- renderText({paste0(c('For example, Nipponbare contains ',Outlier4[which(Outlier4$Genome=="Nipponbare"),]$Size,' bp DNAs mapped onto other places.'), collapse= " ",sep = "")});
}
...
gff3_Coor<-read.table("Oryza_sativa_Nipponbare_7.0_working_gene_coordinate",header=F);
gff3_Coor1<-gff3_Coor[which(gff3_Coor$V3==Gene),][,1:2]; # annotation in GFF3 (only gene)

Mapped0<-data.frame(Filtered_output2[which(Filtered_output2$Genome=="Nipponbare"),][,6],Filtered_output2[which(Filtered_output2$Genome=="Nipponbare"),][,7]);
...
output$text4 <- renderText({paste0(c('Submitted gene (Nipponbare) annotated between: ', gff3_Coor1 ,'; Plotted CDS (Nipponbare) contained between: ',Mapped1), collapse= " ",sep = "")});

######################################################################## To search for the outliers!


system("perl extract_syn_fa_Rice_1_06_01_Test4_copy_3.pl 2");
system("rm extract_syn_fa_Rice_1_06_01_Test4_copy_3.pl");
