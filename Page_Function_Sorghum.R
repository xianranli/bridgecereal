...
page_3 <- function(){
  ...
  page(
    ...
    ui <- function(request){
      tagList(
        h1("This is Shorghum genome page",style="text-align:center"),
        nav_links,


# To add ui part for page3   

useShinyjs(),

column(8,offset = 5, titlePanel("Shorghum Alignment output")), # titlePanel

sidebarLayout(

sidebarPanel(

column(12,textInput("Gene","Gene Name (Sobic010G276200 or others)")),
...
pickerInput(
  inputId = "id", 
  label = "Genomes (Select BTx623 and others) :", 
  choices = c('353', 'AusTRCF317961', 'BTx623', 'BTx642', 'IS929', 'IS3614',  'IS8525',  'IS12661',  'IS19953',  'Ji2731', 'PI525695', 'PI532566', 'PI536008', 'R931945', 'Rio', 'RTx430', 'S369', 'SC187'),
  options = list(
    'actions-box' = TRUE, 
    size = 18,
...
fluidRow(
  ...
  #   column(4, plotOutput("plot_tree")),
   ...
   # verbatimTextOutput("info")
    ...
    }, # For ui function of page_3



# To add server function part for page3
...
dna <- readDNAStringSet("Sbicolor_BTx623_v3.1.1.cds_1.fa"); ## Try CDS.1
...
##############
system("cat extract_syn_fa_Sorghum_1_06_01.pl > extract_syn_fa_Sorghum_1_06_01_Test4_copy_3.pl")
...
replace_pattern_in(file_contents="gene='Sobic010G276200'",replace=Gene_update,file_pattern="Test4_copy_3",file_contents_perl = TRUE);


##############
system("perl extract_syn_fa_Sorghum_1_06_01_Test4_copy_3.pl 1");


######################################################################## To search for the outliers!
result_folder1<-c('/mnt/946c1663-fcbd-4a78-8887-c55f23c5b496/bszhang/Sorghum/BTx623/Candidate_genes/');
...
output$text3 <- renderText({paste0(c('For example, BTx623 contains ',Outlier4[which(Outlier4$Genome=="BTx623"),]$Size,' bp DNAs mapped onto other places.'), collapse= " ",sep = "")});
}
...
gff3_Coor<-read.table("Sbicolor_BTx623_v3.1.1_working_gene_coordinate",header=F);
gff3_Coor1<-gff3_Coor[which(gff3_Coor$V3==Gene),][,1:2]; # annotation in GFF3 (only gene)

Mapped0<-data.frame(Filtered_output2[which(Filtered_output2$Genome=="BTx623"),][,6],Filtered_output2[which(Filtered_output2$Genome=="BTx623"),][,7]);
...
output$text4 <- renderText({paste0(c('Submitted gene (BTx623) annotated between: ', gff3_Coor1 ,'; Plotted CDS (BTx623) contained between: ',Mapped1), collapse= " ",sep = "")});

######################################################################## To search for the outliers!


system("perl extract_syn_fa_Sorghum_1_06_01_Test4_copy_3.pl 2");
system("rm extract_syn_fa_Sorghum_1_06_01_Test4_copy_3.pl");
