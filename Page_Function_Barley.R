...
page_2 <- function(){
  ...
  page(
    ...
    ui <- function(request){
      tagList(
        h1("This is Barley genome page",style="text-align:center"),
        nav_links,


# To add ui part for page2   

useShinyjs(),

column(8,offset = 5, titlePanel("Barley Alignment output")), # titlePanel

sidebarLayout(

sidebarPanel(

column(12,textInput("Gene","Gene Name (1H01G000100 or others)")),

# column(6,textInput("Gene_ID","Sobic")),

column(12,textInput("Chr","Chromosome (chr1H or others)")),
...
pickerInput(
  inputId = "id", 
  label = "Genomes (Select Morex and others) :", 
  choices = c('Akashinriki', 'B1K', 'Barke', 'GoldenPromise', 'Hockett', 'HOR10350',  'HOR13821',  'HOR13942',  'HOR21599',  'HOR3081', 'HOR3365', 'HOR7552', 'HOR8148', 'HOR9043', 'Igri', 'Morex', 'OUN333', 'RGTPlanet', 'ZDM01467', 'ZDM02064'),
  options = list(
    'actions-box' = TRUE, 
    size = 21,
...
fluidRow(
  ...
  #   column(4, plotOutput("plot_tree")),
   ...
   # verbatimTextOutput("info")
    ...
    }, # For ui function of page_2



# To add server function part for page2
...
dna <- readDNAStringSet("Morex.cds_1.fa"); ## Try CDS.1
...
##############
system("cat extract_syn_fa_Barley_1_06_01.pl > extract_syn_fa_Barley_1_06_01_Test4_copy_3.pl")
...
replace_pattern_in(file_contents="target_ch='chr1H'",replace=Chr_update,file_pattern="Test4_copy_3",file_contents_perl = TRUE);

Gene_update0 <- paste('gene=',input$Gene,sep = "'");  ## from shiny input
Gene_update <- paste(Gene_update0,symbol,sep = "");
replace_pattern_in(file_contents="gene='1H01G000100'",replace=Gene_update,file_pattern="Test4_copy_3",file_contents_perl = TRUE);


##############
system("perl extract_syn_fa_Barley_1_06_01_Test4_copy_3.pl 1");


######################################################################## To search for the outliers!
result_folder1<-c('/mnt/946c1663-fcbd-4a78-8887-c55f23c5b496/bszhang/Barley/Morex/Candidate_genes/');
...
output$text3 <- renderText({paste0(c('For example, Morex contains ',Outlier4[which(Outlier4$Genome=="Morex"),]$Size,' bp DNAs mapped onto other places.'), collapse= " ",sep = "")});
}
...
gff3_Coor<-read.table("Morex_gene_working_gene_coordinate",header=F);
gff3_Coor1<-gff3_Coor[which(gff3_Coor$V3==Gene),][,1:2]; # annotation in GFF3 (only gene)

Mapped0<-data.frame(Filtered_output2[which(Filtered_output2$Genome=="Morex"),][,6],Filtered_output2[which(Filtered_output2$Genome=="Morex"),][,7]);
...
output$text4 <- renderText({paste0(c('Submitted gene (Morex) annotated between: ', gff3_Coor1 ,'; Plotted CDS (Morex) contained between: ',Mapped1), collapse= " ",sep = "")});

######################################################################## To search for the outliers!


system("perl extract_syn_fa_Barley_1_06_01_Test4_copy_3.pl 2");
system("rm extract_syn_fa_Barley_1_06_01_Test4_copy_3.pl");
